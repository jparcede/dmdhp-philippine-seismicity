...
def main():
    ...
    for seq in SEQUENCES:
        seq_id = seq["seq_id"]
        catalog_path = os.path.join(CATALOG_DIR, f"{seq_id}_catalog.csv")
        ...
        lo, hi, n_boot_ok = bootstrap_ci_2zone(th2, T, m0, n_boot=N_BOOT,
                                                seed=seed + 111)
        print(f"  Successful bootstrap refits: {n_boot_ok}/{N_BOOT}")
        R_lo = (lo[2] / hi[3]) if (np.isfinite(lo[2]) and np.isfinite(hi[3]) and hi[3] > 0) else np.nan
        R_hi = (hi[2] / lo[3]) if (np.isfinite(hi[2]) and np.isfinite(lo[3]) and lo[3] > 0) else np.nan
